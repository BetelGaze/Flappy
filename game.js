...
cvs.addEventListener("click",function(evt){
  ...
  switch(state.current){
    ...
    case state.over:
      ...
      //CHECK IF WE CLECK N THE START BUT
...
const bird = {
  ...
  flap: function(){
    ...
    update: function(){
    //if games state get ready flap slow each 10
    //else 5
    this.period = state.current ==state.getReady ? 10 : 5;

    //increment frame by 1 eah per
    this.frame += frames%this.period == 0 ? 1 : 0;

    //frame foes from 0 to 4, then again 0
    this.frame = this.frame%this.animation.length;
      ...
      if(state.current == state.getReady){
        ...
        //if speed is > jump , meens bird is falling down
...
if(bird.x + bird.radius > p.x && bird.x - bird.radius < p.x +this.w &&
  ...
  p.x -=this.dx;
      ...
      //if pipes go beyond of the canvas, we delete them from the array
      if(p.x +this.w <=0){
        this.position.shift();
        score.value +=1;
